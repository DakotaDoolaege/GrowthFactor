	...
	public class PlayerInstantiator : Instantiator
	{
		...
		public override void Start()
		{
			...
			for (int i = 0; i < GameVariables.PlayerStations.Count; i++)
			{
				...
				{
					startPosition.x = startPosition.x - 920 + offset.x;
					startPosition.y = startPosition.y - 540 + offset.y;
		...
		private Vector2 GetOffset(Vector2 position)
		{
			...
			{
				if (position.x <= 960 && position.y < 540)//bottom left
				{
					return new Vector2(-125, 0);
				}
				else if (position.x >= 960 && position.y < 540)//bottom right
				{
					return new Vector2(-125, 0);
				}
				else if (position.x <= 960 && position.y > 540)//top left
				{
					return new Vector2(50, 0);
				}
				else if (position.x >= 960 && position.y > 540)//top right
				{
					return new Vector2(50, 0);
				}
				else if (position.x >= 960 && position.y == 540) //right center
				{
					return new Vector2(-38, -100);
				}
				return new Vector2(-38, 100);//left center
